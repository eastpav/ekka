...
discover(Options) ->
    ...
    case k8s_service_get(Server, Service, Namespace) of
        {ok, Response} ->
            Addresses = 
            case extract_addresses(AddrType, Response, Namespace) of
                [] ->
                    extract_not_ready_addresses(AddrType, Response, Namespace);
...
% 若需要emq节点成功加入集群后才被分发流量，则增加notReadyAddresses的提取
extract_not_ready_addresses(Type, Response, Namespace) ->
    lists:flatten(
      [[ extract_host(Type, Addr, Namespace)
         || Addr <- maps:get(<<"notReadyAddresses">>, Subset, [])]
            || Subset <- maps:get(<<"subsets">>, Response, [])]).
